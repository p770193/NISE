...
class Omega:
    ...
    mu_cb = 1.0 * mu_ag
    #--------------------------Recorded attributes--------------------------
    out_vars = ['dm_vector', 'out_group', 'rho_0', 'Da',
    ...
    def _gen_matrix(self, E1, E2, E3, t, wl, w1first = True):
        ...
        O[:,2,0] =  1   * mu_bg  * first  * rotor(-wbg*t)
        O[:,3,0] = -D01 * mu_ag  * E2     * rotor( wag*t)
        O[:,4,0] = -1   * mu_bg  * E2     * rotor( wbg*t)
        # from ag1
        O[:,5,1]  = -1   * mu_ag  * E2     * rotor( wag*t)
        O[:,7,1]  = -1   * mu_bg  * E2     * rotor( wbg*t)
        O[:,9,1]  =  D12 * mu_ag  * second * rotor(-w2aa*t)
        O[:,11,1] =  1   * mu_bg  * second * rotor(-wca*t)
        # from bg1
        O[:,6,2]  = -1   * mu_bg  * E2     * rotor( wbg*t)
        # state filling for ZQC and/or DQC?  Depends on whether I think all
        # 1S excitons can couple with any b state...
        O[:,8,2]  = -D01 * mu_ag  * E2     * rotor( wag*t)
        O[:,10,2] =  1   * mu_bg  * second * rotor(-w2bb*t)
        O[:,11,2] =  D01 * mu_ag  * second * rotor(-wcb*t)
        # from ga
        O[:,5,3] =  1 * mu_ag  * first  * rotor(-wag*t)
        O[:,8,3] =  1 * mu_bg  * first  * rotor(-wbg*t)
        # from gb
        O[:,6,4] =  1   * mu_bg  * first  * rotor(-wbg*t)
        O[:,7,4] =  D01 * mu_ag  * first  * rotor(-wag*t)
        # from aa (/ gg)
        O[:,14,5] =  D12 * mu_2aa * second * rotor(-w2aa*t) * mu_2aa
        O[:,12,5] =  1   * mu_ca  * second * rotor(-wca*t)  * mu_ca
        O[:,17,5] = -1   * mu_bg  * second * rotor(-wbg*t)  * mu_bg
        #   twice the weight because of gamma and alpha pathways
        O[:,16,5] = -mu_ag  * second * rotor(-wag*t)  * mu_ag * (D01 + D10)
        # from bb (/ gg)
        O[:,15,6] =  1 * mu_2bb * second * rotor(-w2bb*t) * mu_2bb
        O[:,13,6] =  D01 * mu_cb  * second * rotor(-wcb*t)  * mu_cb
        O[:,16,6] = -D01 * mu_ag  * second * rotor(-wag*t)  * mu_ag
        #   twice the weight because of gamma and alpha pathways
        O[:,17,6] = -mu_bg  * second * rotor(-wbg*t)  * mu_bg * 2
        # from ab
        ...
        # from 2bg
        O[:,15,10] = -mu_bg  * E2     * rotor(wbg*t)  * mu_2bb
        O[:,16,10] =  mu_2bb * E2     * rotor(w2bb*t) * mu_bg
        # from cg
